...
public class EditAccountServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (da.checkDuplicate(phone, email, acc.getUsId())) {
                request.setAttribute("error", "Thông tin này đã được người dùng khác sử dụng.");
                request.getRequestDispatcher("editacc.jsp").forward(request, response);
            } else {
                da.updateUser(acc.getUsId(), name, email, phone, address);
                acc.setUsName(name);
                acc.setPhone(phone);
                acc.setMail(email);
